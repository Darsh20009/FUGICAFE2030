...
const SETTINGS_KEY = 'fuji-printer-settings';
const DEVICE_KEY   = 'fuji-printer-device';
...
const PROFILES_KEY = 'fuji-printer-profiles';
...
export function buildEscPosReceipt(data: EscPosReceiptData): Uint8Array {
  ...
  const _pmLabel = (() => {
    ...
    if (m === 'loyalty' || m === 'qahwa-card' || m === 'fuji-card') return 'بطاقة ولاء';
  ...
  buf.push(...textBytes('فوجي كافيه'), 0x0a);
...
const BT_DEVICE_KEY = 'fuji-bt-printer';
...
export async function buildRefundCanvas(opts: RefundReceiptOpts): Promise<HTMLCanvasElement> {
  ...
  const payLabel = (() => {
    ...
    if (m === 'geidea' || m === 'paymob' || m === 'paymob-card') return 'بطاقة ائتمان';
    if (m === 'split') return 'نقدي + شبكة';
    if (m === 'loyalty' || m === 'qahwa-card' || m === 'fuji-card' || m === 'loyalty-card') return 'بطاقة ولاء';
