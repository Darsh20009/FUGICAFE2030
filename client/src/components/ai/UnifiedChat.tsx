...
const LOGO_LIGHT = "/fuji-logo.png";
...
const WHATSAPP_URL = "https://api.whatsapp.com/send?phone=966552469643";
...
export const UnifiedChat = memo(function UnifiedChat() {
  ...
  const openChat = useCallback((tab: TabType) => {
    ...
    if (tab === "advisor" && advisorMessages.length === 0) {
      setAdvisorMessages([{
        role: "assistant",
        content: "أهلاً بك في فوجي كافيه ☕ أنا مستشارك الشخصي للقهوة — أخبرني عن ذوقك أو المناسبة وسأقترح لك القهوة المثالية.\n\nWelcome to Fuji Cafe ☕ I am your personal coffee advisor — tell me your taste or the occasion (in Arabic or English) and I'll suggest the perfect coffee."
      }]);
    }
    if (tab === "support" && supportMessages.length === 0) {
      setSupportMessages([{
        role: "assistant",
        content: "مرحباً بك في الدعم الفني لفوجي كافيه. كيف يمكنني خدمتك؟\n• تتبع طلبك  • معلومات المنتجات  • سياسة الاسترجاع  • استفسارات أخرى\n\nHi! Welcome to Fuji Cafe support. How can I help?\n• Track your order  • Product info  • Return policy  • Other questions\n\n(You can write in Arabic or English — I'll reply in the same language.)"
      }]);
    }
  ...
  const quickReplies = useMemo<string[]>(() => {
    ...
    if (activeTab === "advisor") {
      ...
      if (messages.length <= 1) {
        return ["☕ قهوة للصباح", "👑 قهوة للمناسبات", "🌿 قهوة إثيوبية", "✨ قهوة خفيفة منعشة", "💫 أفضل قهوة هدية"];
      }
      // After product recommendations
      if (hasProducts) {
        return ["💰 أرني أرخص الخيارات", "👑 أرني الأفخم", "🔄 اقترح قهوة أخرى", "🌿 حبوب إثيوبية", "☕ مشروب بارد"];
      }
      // Generic follow-up
      return ["💡 ما الفرق بينهم؟", "💰 ما الأسعار؟", "📦 هل متوفر؟", "🎁 قهوة هدية مميزة", "🌿 قهوة برازيلية"];
    }
    // Support
    if (messages.length <= 1) {
          ...
          >
            ...
            <div className="p-3 space-y-2 bg-white">
              ...
              <button
                ...
                className="group w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-[#E8637A]/5 transition-all active:scale-[0.98] border border-transparent hover:border-[#E8637A]/20"
              >
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-md" style={{ background: "linear-gradient(135deg, #f08a9d, #d44f66)" }}>
                  <Sparkles className="h-5 w-5 text-white" />
                </div>
                <div className="flex-1 text-right min-w-0">
                  <p className="font-black text-sm text-[#6B3F2A]">مستشار القهوة</p>
                  <p className="text-[11px] text-gray-400 font-medium truncate">اعثر على قهوتك المثالية بالذكاء</p>
                </div>
                <span className="text-[9px] font-black text-[#E8637A] bg-[#E8637A]/10 px-2 py-1 rounded-full">AI</span>
              </button>
            ...
            <div className="px-4 py-3 bg-gradient-to-l from-[#FFFFFF] to-white border-t border-[#E8637A]/10 text-center">
              <p className="text-[10px] text-gray-400 font-bold tracking-wide">
                فوجي كافيه <span className="text-[#E8637A]">·</span> Fuji Cafe
              </p>
            </div>
          </motion.div>
        )}
              ...
              <div className="flex items-center gap-2.5">
                <div>
                  <h4 className="font-black text-sm text-[#6B3F2A] text-center">
                    {isAdvisor ? "فوجي — مستشار القهوة" : "الدعم الفني"}
                  </h4>
            ...
            <div className="px-4 py-3 border-t border-gray-100 bg-white">
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder={isAdvisor ? "اسألنا عن القهوة… / Ask about coffee…" : "اكتب رسالتك… / Type…"}
              ...
              <p className="text-[9px] text-gray-400 text-center mt-2 font-bold tracking-wide">
                مدعوم بالذكاء الاصطناعي · فوجي كافيه
              </p>
            </div>
          </motion.div>
          </>
