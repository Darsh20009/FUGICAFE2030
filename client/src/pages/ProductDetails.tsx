...
export default function ProductDetails() {
  ...
  return (
    <Layout>
      <SEO
        title={`${product.name}${(product as any).nameEn ? ` | ${(product as any).nameEn}` : ""} — فوجي كافيه`}
        description={`اشتري ${product.name} من متجر فوجي كافيه Fuji Cafe. ${(product as any).description ? String((product as any).description).slice(0, 120) : "قهوة متخصصة فاخرة"} — توصيل سريع لجميع مدن السعودية.`}
        keywords={`${product.name}, ${(product as any).nameEn || ""}, قهوة متخصصة, فوجي كافيه, Fuji Cafe, قهوة سعودية, شراء قهوة اونلاين, ${product.name} سعر`}
