/**
 * Email Service — Fuji Cafe
 * Powered by SMTP2GO API
 * All templates are Arabic RTL with Fuji Cafe branding
 */
...
function getCredentials() {
  ...
  return {
    apiKey,
    sender: process.env.EMAIL_SENDER || "fugi2030@outlook.com",
    senderName: process.env.EMAIL_SENDER_NAME || "فوجي كافيه",
  };
}
          ...
          <tr>
            <td align="center" style="background-color:#1A1A1A;padding:32px 40px;">
              <p style="margin:0 0 12px;color:rgba(255,255,255,0.5);font-size:11px;font-weight:600;line-height:1.8;font-family:'Segoe UI',Tahoma,Arial,sans-serif;">
                &copy; ${new Date().getFullYear()} فوجي كافيه &mdash; جميع الحقوق محفوظة
              </p>
...
export async function sendOrderConfirmationEmail(params: {
  ...
  const content = `
    ...
    <p style="margin:24px 0 0;padding-top:24px;border-top:1px solid rgba(0,0,0,0.06);font-size:12px;color:rgba(0,0,0,0.55);line-height:1.7;font-family:'Segoe UI',Tahoma,Arial,sans-serif;">
      هل لديك استفسار؟ تواصل معنا على <a href="mailto:fugi2030@outlook.com" style="color:#2d1a14;font-weight:800;text-decoration:none;">fugi2030@outlook.com</a>
    </p>
  `;
  ...
  return sendEmail({
    ...
    subject: `✅ تم استلام طلبك #${params.orderRef} | Order #${params.orderRef} Received — Fuji Cafe`,
    html: baseTemplate(`تأكيد الطلب #${params.orderRef} / Order Confirmation`, content + enMirror),
    text: `تم استلام طلبك #${params.orderRef} بقيمة ${params.total} ر.س. شكراً لتسوقك مع فوجي كافيه.\n\nYour order #${params.orderRef} (${params.total} SAR) has been received. Thank you for shopping with Fuji Cafe.`,
    attachments,
  });
}
  ...
  const statusConfigs = {
    ...
    cancelled: {
      ...
      message: `
        ...
        <p>إذا كنت قد دفعت ولم تتلقَّ استرداداً، يرجى التواصل معنا فوراً على <a href="mailto:fugi2030@outlook.com" style="color:#000;font-weight:800">fugi2030@outlook.com</a></p>
      `,
      cta: "تواصل معنا",
    },
  };
...
export async function sendWelcomeEmail(params: {
  ...
  const content = `
    <div style="text-align:center;margin-bottom:32px;">
      ...
      <p style="margin:0;font-size:14px;color:rgba(0,0,0,0.55);font-weight:600;font-family:'Segoe UI',Tahoma,Arial,sans-serif;">انضممت إلى عائلة فوجي كافيه</p>
    </div>
  ...
  return sendEmail({
    to: params.to,
    toName: params.customerName,
    subject: `👋 أهلاً ${params.customerName}! مرحباً بك في فوجي كافيه | Welcome to Fuji Cafe`,
    html: baseTemplate("مرحباً بك في فوجي كافيه / Welcome to Fuji Cafe", content + enMirror),
    text: `أهلاً ${params.customerName}! مرحباً بك في فوجي كافيه.\nWelcome ${params.customerName}! Your Fuji Cafe account is ready.`,
  });
}
...
export async function sendPasswordResetEmail(params: {
  ...
  return sendEmail({
    to: params.to,
    toName: params.customerName,
    subject: `🔐 استعادة كلمة المرور | Password Reset — Fuji Cafe`,
...
export async function sendAdminAlertEmail(params: {
  ...
  const content = `
    ...
    <p style="margin:0 0 24px;font-size:13px;color:rgba(0,0,0,0.55);font-weight:600;font-family:'Segoe UI',Tahoma,Arial,sans-serif;">تنبيه إداري — فوجي كافيه</p>
  ...
  return sendEmail({
    to: params.to,
    subject: params.subject.includes("|") ? params.subject : `${params.subject} | Fuji Cafe Admin Alert`,
...
export async function sendActivationEmail(params: {
  ...
  const content = `
    <div style="text-align:center;margin-bottom:32px;">
      <div style="font-size:48px;margin-bottom:12px;line-height:1;">🎉</div>
      <h1 style="margin:0 0 8px;font-size:26px;font-weight:900;color:#000;font-family:'Segoe UI',Tahoma,Arial,sans-serif;">مرحباً بك في فريق فوجي كافيه</h1>
    ...
    <p style="margin:0 0 16px;font-size:14px;color:rgba(0,0,0,0.75);line-height:1.8;font-family:'Segoe UI',Tahoma,Arial,sans-serif;">
      ...
      تم إنشاء حسابك في نظام فوجي كافيه. لتفعيل حسابك وتعيين كلمة المرور الخاصة بك، اضغط على الزر أدناه:
    </p>
  ...
  const enMirror = englishMirror(`
    <h2 style="margin:0 0 6px;font-size:20px;font-weight:900;color:#000000;">🎉 Welcome to the Fuji Cafe Team</h2>
    <p style="margin:0 0 12px;font-size:13px;color:rgba(0,0,0,0.55);font-weight:600;">Your account has been created as ${enRole}</p>
    <p style="margin:0 0 12px;font-size:13px;color:rgba(0,0,0,0.7);line-height:1.7;">Hi ${params.name},<br/>Your Fuji Cafe staff account has been created. To activate it and set your password, click the link below:</p>
  ...
  return sendEmail({
    to: params.to,
    toName: params.name,
    subject: `🎉 تفعيل حسابك في فوجي كافيه | Activate your Fuji Cafe account`,
 ...
 * From: info@fuji.cafe  →  To: firstrafiff@gmail.com
 */
...
<table class="container" role="presentation" border="0" cellspacing="0" cellpadding="0" width="680" style="max-width:680px;background:#ffffff;border-radius:6px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
  ...
  <tr>
    <td align="center" style="background:${NAVY};background-image:linear-gradient(135deg,#1A1A1A 0%,${NAVY} 50%,#3d261e 100%);padding:28px 40px;border-bottom:3px solid ${GOLD};">
      <div style="color:${GOLD};font-size:12px;font-weight:900;letter-spacing:0.4em;text-transform:uppercase;margin-bottom:6px;">فوجي كافيه — لوحة الإدارة</div>
  ...
  <tr>
    <td align="center" style="background:#1A1A1A;padding:24px 40px;border-top:1px solid rgba(201,169,110,0.2);">
      <div style="color:${GOLD};font-size:10px;font-weight:700;letter-spacing:0.35em;text-transform:uppercase;margin-bottom:6px;">Fuji Cafe — Admin Notification</div>
