    ...
    if (order.shippingMethod === "delivery" && isShipoxConfigured()) {
      enqueueJob("paid-shipox-create", async () => {
        try {
          ...
          const senderName    = (settings as any)?.storeName    || "فوجي كافيه";
...
${allUrls.map(u => `  <url>
  ...
  app.post("/api/auth/forgot/init", async (req, res) => {
    try {
      ...
      const hasEmail = !!(user.email && /^\S+@\S+\.\S+$/.test(user.email) && !user.email.endsWith("@fuji.cafe"));
  ...
  app.get("/api/admin/email/status", checkPermission("settings.manage"), (_req, res) => {
    res.json({
      ...
      sender: "fugi2030@outlook.com",
      senderName: "فوجي كافيه",
      provider: "SMTP2GO",
    });
  });
        ...
        if (phone.length < 8) {
          ...
          if (existing && existing.role !== "customer") {
            ...
            const baseUserData: any = {
              ...
              phone,
              username: phone,
              email: branchData.email || `${phone}@fuji.cafe`,
              password: hashed,
              role: "employee",
              branchId: branch.id,
              loginType: "dashboard",
  ...
  app.post("/api/admin/reviews/:id/reply", checkPermission("orders.view"), async (req, res) => {
    try {
      ...
      const r = await storage.replyToReview(req.params.id, {
        ...
        byName: user.name || "إدارة فوجي كافيه",
      });
      ...
      const r = await fetch(`${base}/v1/intention/`, {
        ...
        body: JSON.stringify({
          ...
          billing_data: {
            ...
            phone_number: "0500000000", email: "fugi2030@outlook.com",
  ...
  app.post("/api/admin/shipox/create/:orderId", async (req, res) => {
    ...
    try {
      ...
      const result = await createShipoxOrder(order, serviceType, {
        senderName:    req.body.senderName    || (settings as any)?.storeName    || "فوجي كافيه",
  ...
  app.post("/api/admin/shipox/return/:orderId", async (req, res) => {
    ...
    try {
      ...
      const result = await createShipoxReturn(order, {
        senderName:    (settings as any)?.storeName    || "فوجي كافيه",
  ...
  app.get("/api/orders/:id/zatca-qr", async (req, res) => {
    try {
      ...
      const result = await buildZatcaQrDataUrl({
        sellerName: settings?.storeNameAr || settings?.storeName || "فوجي كافيه",
      ...
      res.json({
        ...
        sellerName: settings?.storeNameAr || "فوجي كافيه",
        vatNumber: settings?.vatNumber || "",
        total: Number(order.total) || 0,
        vatAmount: Number(order.vatAmount) || 0,
  ...
  app.post("/api/admin/inbox/ai-compose", inboxAccess, async (req, res) => {
    try {
      ...
      let systemPrompt = `أنت مساعد كتابة بريد إلكتروني محترف لشركة فوجي كافيه الفاخرة. 
...
لا تضف أي تفسيرات أو عناوين، أعد فقط نص البريد جاهزاً للإرسال.`;
  ...
  app.post("/api/admin/inbox/ai-summarize", inboxAccess, async (req, res) => {
    try {
      ...
      const systemPrompt = `أنت مساعد ذكي لصندوق بريد شركة فوجي كافيه الفاخرة.
