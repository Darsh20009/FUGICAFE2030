...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: "Send a personalised email to a customer (auto-wrapped in the Fuji Cafe luxury template).",
      parameters: {
        type: "object",
        properties: {
...
<div class="header"><img src="${LOGO_BASE64}" alt=""/><div class="brand">فوجي كافيه</div><div class="sub">فوجي كافيه · FUJI CAFE</div></div>
<div class="body">${args.messageHtml}</div>
<div class="footer">© ${new Date().getFullYear()} فوجي كافيه — جميع الحقوق محفوظة</div>
</div></body></html>`;
...
const SYSTEM_PROMPT_AR = (today: string, role: string, name: string) => `أنت "لمسة" ☕ — مساعدة موظفي فوجي كافيه الذكية والذكية جداً.

🎯 **هويتك ومهمتك:**
- اسمك "لمسة"، خبيرة تشغيل متجر قهوة متخصصة، عملية وسريعة وذكية
...
▸ **"غيّر سعر قهوة كذا إلى ٢٥٠"** → search_products("كذا") → update_product(productId, price:250)
...
- تحللين البيانات بنظرة ذكية: "أكثر العملاء يطلبون قهوة إثيوبية الجمعة — الأسبوع القادم لو ركّزنا الإعلان ليلة الخميس؟"
...
const SYSTEM_PROMPT_EN = (today: string, role: string, name: string) => `You are "Lamsa" 🌸 — the smart, capable AI assistant for Fuji Cafe staff.

🎯 **Identity & mission:**
- You're "Lamsa", an expert specialty-coffee store operator — practical, fast, and smart
...
▸ **"Change the price of coffee X to 250"** → search_products("X") → update_product(productId, price:250)
